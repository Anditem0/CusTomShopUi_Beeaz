...
class Shop extends PluginBase
{
                    ...
                    $worlds = $this->getConfig()->get("Worlds");
                        if(in_array($level, $worlds) === true){
                            ...
                            if (empty($args)) {
                                ...
                                $cfg = $cfg[$ans];
                                if ($sender->isCreative() and $this->getConfig()->get("Survival") === true) {
                                    $msg = new Config($this->getDataFolder() . "messages.yml", Config::YAML);
    ...
    public function Category($cfg, Player $player, Config $msg, $ans): void
    {
        ...
        if ($this->getConfig()->get("Category_ExitButton") === true) {
            $form->addButton("§l§e‹ §6Thoát §e‹", 1 , "https://icons-for-free.com/iconfiles/png/512/cross+exit+remove+icon-1320161389317562876.png");
        }
    ...
    public function Items(Player $player, $categorys, $cfg, $ans)
    {
        ...
        $form->setTitle("§l§c✢ §6ｓｈｏｐ§c ✢");
        $money = EconomyAPI::getInstance()->myMoney($player->getName());
         $form->setContent("§l§a+ §e Xu của bạn:§e " . $money);
        if ($this->getConfig()->get("Items_ExitButton") === true) {
            $form->addButton("§l§e‹ §6Thoát §e‹", 1 , "https://icons-for-free.com/iconfiles/png/512/cross+exit+remove+icon-1320161389317562876.png");
        }
            $items = $cfg[$categorys];
            if (!isset($items["Sub"])) {
                foreach ($items["Items"] as $cate => $item) {
                    $list = explode(":", $item);
                    if ($list[5] == "Default") {
                        $name = ItemFactory::getInstance()->get((int)$list[0], (int)$list[1], 1)->getVanillaName();
                    } else {
                        $name = $list[5];
                    }
                    if ($list[0] == "cmd") {
                        ...
                        if (substr($list[5], 0, 4) == "http") {
                            $form->addButton("§c§l› §6".$list[1]." " . $msg2." §ec‹", 1, $list[5] . ":" . $list[6]);
                        } else {
                            $form->addButton("§c§l› §6".$list[1] . " " . $msg2. " §c‹", 0, $list[5]);
                        }
                    } else {
                        $msg2 = str_replace("{price}", "$list[3]", $msg->getNested("Messages.Each"));
                        if (substr($list[6], 0, 4) == "http") {
                            $form->addButton("§c§l› §6".$name . " " . $msg2." §c‹", 1, $list[6] . ":" . $list[7]);
                        } else {
                            $form->addButton("§c§l› §6".$name . " " . $msg2." §c‹", 0, $list[6]);
                        }
                    }
                }
            } else {
    ...
    public function SubItems(Player $player, $categorys, $cfg, $ans)
    {
        ...
        $msg = new Config($this->getDataFolder() . "messages.yml", Config::YAML);
        if ($this->getConfig()->get("Items_ExitButton") === true) {
            $form->addButton(" §l§e› §cThoát §e‹", 1 , "https://icons-for-free.com/iconfiles/png/512/cross+exit+remove+icon-1320161389317562876.png");
        }
         $money = EconomyAPI::getInstance()->myMoney($player->getName());
         $form->setContent("§l§e→§f Xu:§e " . $money);
            $items = $cfg[$categorys];
        ...
        $form = new CustomForm(function (Player $player, $data) use ($cfg, $categorys, $item, $ans, $sub) {
            if ($data === null) {
                ...
                foreach ($vars as $var => $replacement) {
                    $message = str_replace((string)$var, (string)$replacement, (string)$message);
                }
                    ...
                    if ($data[1] == false) {
                        if ($money >= $list[3] * $data1) {
                            ...
                            foreach ($tags as $tag => $replacement) {
                                $message = str_replace((string)$tag,(string) $replacement, (string)$message);
                            }
                            $player->sendMessage($message);
                        }
                    }
        ...
        if ($item !== null) {
            ...
            if ($this->getConfig()->getNested("Types.Toggle") == true) {
                ...
                foreach ($vars2 as $var => $replacement) {
                    $message2 = str_replace((string)$var, (string)$replacement, (string)$message2);
                }
